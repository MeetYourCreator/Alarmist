export const formatZoneName = (zoneStr) => {
  ...
  const firstForwardSlashRemovedArr = zoneStr2
    .slice(indexOfFirstForwardSlash)
    .replace('/', '')
  ...
  if (indexOfSecondForwardSlash >= 0) {
    const country1 = firstForwardSlashRemovedArr
      .slice(indexOfSecondForwardSlash)
      .replace('/', ' ')
      .replace('_', '  ')
      .replace('_', '  ');
    return country1;
  } else {
    const country2 = firstForwardSlashRemovedArr
      .replace('_', ' ')
      .replace('_', ' ');
    return country2;
  }
